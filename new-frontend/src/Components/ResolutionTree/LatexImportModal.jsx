...
const LatexImportModal = ({ isOpen, onClose, onImport }) => {
  ...
  const handleImport = async () => {
    if (!latexCode.trim()) {
      toast.error(t('resolution.import_modal_enter'));
      return;
    }
    if (!validation.isValid) {
      toast.error(t('resolution.import_modal_fix'));
      return;
    }
    ...
    try {
      ...
      onImport(result);
      toast.success(t('resolution.import_success'));
      handleClose();
    } catch (error) {
      toast.error(t('resolution.import_error', { message: error.message }));
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] flex flex-col">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-800">{t('resolution.import_modal_title')}</h2>
          <button
            onClick={handleClose}
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                placeholder={t('resolution.import_modal_placeholder')}
